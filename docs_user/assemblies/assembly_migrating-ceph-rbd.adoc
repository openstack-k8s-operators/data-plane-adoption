...
To migrate Red Hat Ceph Storage RBD, your environment must meet the following requirements:
...
mon and mgr daemons. For the purposes of this POC, you will deploy a Red Hat Ceph Storage cluster
...
* You can keep the mon IP addresses moving them to the Red Hat Ceph Storage nodes.
...
_admin nodes that can be used by administrators to manage the Red Hat Ceph Storage cluster
...
network to make sure that you can use both Red Hat Ceph Storage public and cluster networks.
...
Create a Red Hat Ceph Storage spec based on the default roles with the mon/mgr on the
controller nodes.
