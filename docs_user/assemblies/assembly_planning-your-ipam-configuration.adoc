...
In a {rhos_long} deployment, each service that is deployed on the {rhocp_long} worker nodes requires an IP address from the IPAM pool. In a {rhos_prev_long} ({OpenStackShort}) deployment, all services that are hosted on a Controller node shared the same IP address.

Since the new control plane deployment has different requirements as to the
number of IP addresses available for services, you cannot reuse the existing IP ranges in the {rhos_acro} environment, depending on the size.
