...
If you are using the Object Storage API of the Ceph Object Gateway (RGW), you can skip this chapter and run Ceph Migration. If your are not planning to migrate Ceph daemons out of controller nodes, it is required to perform the steps described by chapters xref:deploying-a-ceph-ingress-daemon_migrating-ceph-rgw[Deploying a Ceph ingress daemon] and xref:updating-the-object-storage-endpoints_migrating-ceph-rgw[Create/Update the Object Storage service endpoints].
