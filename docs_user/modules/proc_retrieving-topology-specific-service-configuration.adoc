...
export PULL_OPENSTACK_CONFIGURATION_NOVADB_MAPPED_CELLS=$(oc run mariadb-client-1 ${MARIADB_RUN_OVERRIDES} -q --image ${MARIADB_IMAGE} -i --rm --restart=Never -- \
    ...
    'select uuid,name,transport_url,database_connection,disabled from cell_mappings where disabled=0;')
----
...
>             "select host from nova.services where services.binary='nova-compute' and disabled=0 and deleted=0;")
> done
----
