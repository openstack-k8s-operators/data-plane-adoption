...
* You might also want to run the `neutron-dhcp` service on your Networker nodes instead of your Compute nodes. Note that using `neutron-dhcp` with OVN is not generally needed.
...
+
[NOTE]
You can reuse most of the `nodeTemplate` section from the `OpenStackDataPlaneNodeSet` CR that is designated for your `Compute` nodes. However, some of the variables might be unnecessary because of a more limited set of services that are running on `Networker` nodes. Ensure that `edpm_enable_chassis_gw: true` is set to run `ovn-controller` in gateway mode.
+
[source,yaml]
----
$ oc apply -f - <<EOF
