...
[source,yaml]
----
$ oc-n $RHOSO18_NAMESPACE -n $RHOSO18_NAMESPACE patch openstackdataplanenodeset <networker_CR_name> --type='json' --patch='[
  {
    "op": "add",
    "path": "/spec/services/-",
...
+
----
$ oc -n $RHOSO18_NAMESPACE wait --for condition=Ready openstackdataplanedeployment/openstack-pre-adoption-networker --timeout=10m
----
...
+
----
$ oc -n $RHOSO18_NAMESPACE wait --for condition=Ready osdpns/<networker_CR_name> --timeout=30m
----
+
* Replace `<networker_CR_name>` with the name of the CR that you deployed for your Networker nodes, for example, `openstack-networker`.
