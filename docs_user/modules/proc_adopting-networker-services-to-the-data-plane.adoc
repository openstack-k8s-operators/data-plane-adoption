...
If you have Networker nodes in your existing {rhos_long} deployment, adopt them to the {rhos_acro} data plane. You decide which services you want to run on the Networker nodes, and then create a separate `OpenStackDataPlaneNodeSet` custom resource (CR) just for these nodes. You might also decide to implement the following options if they apply to your environment:
...
* Optional: You can run the `neutron-dhcp` service on your Networker nodes instead of your Compute nodes. Using `neutron-dhcp` with OVN is not generally needed. However, it might be necessary in deployments with dynamic host configuration protocol (DHCP) relays, or if you require advanced DHCP options that are supported by dnsmasq but not by the OVN native DHCP implementation.

.Prerequisites

* Define the following shell variable. The following value is an example
and applies to a single node standalone {OpenStackPreviousInstaller} deployment:
+
[subs=+quotes]
----
...
+
[NOTE]
You can reuse most of the `nodeTemplate` section from the `OpenStackDataPlaneNodeSet` CR that is designated for your `Compute` nodes. However, some of the variables might be unnecessary because of a more limited set of services that are running on `Networker` nodes. 
+
[source,yaml]
----
$ oc apply -f - <<EOF
...
. Ensure that the `ovn-controller` settings that are configured in the `OpenStackDataPlaneNodeSet` CR are the same as the settings that were set in the `Networker` nodes before adoption. This configuration is stored in the `external_ids`` column in the `Open_vSwitch` table in the Open vSwitch database:
+
----
ovs-vsctl list Open .
...
...
+
[NOTE]
Alternatively, you can include the `Networker` specific node set in the
`nodeSets` list when you create the main `OpenStackDataPlaneDeployment` CR, before you deploy it. Note that once an `OpenStackDataPlaneDeployment` CR is deployed, you cannot add new node sets to it later.
...
. Verify that the {networking_first_ref} agents are running. The list of agents might vary depending on the services you enabled:
+
----
