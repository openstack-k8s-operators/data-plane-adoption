...
* If the source environment does not use TLS Everywhere, modify
spec:tls:endpoint:internal:enabled to false. Do the same for
spec:tls:endpoint:public:enabled.
