...
* If the source environment has TLS Everywhere enabled, it may be possible to copy over the root CA from the source environment to the new environment to minimize disruption on the compute nodes.
...
* On RHOSO 18, TLS Everywhere is enabled by default. If the source environment does not use TLS Everywhere and TLS Everywhere is not desired for the new environment, modify spec:tls:endpoint:internal:enabled to false.
