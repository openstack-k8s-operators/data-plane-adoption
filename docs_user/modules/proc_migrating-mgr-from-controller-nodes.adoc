...
+
[NOTE]
Default dashboard port for ceph-mgr in greenfield deployment is 8443. With director-deployed Ceph, the default port is 8444 because the service ran on the controller node and it was necessary to use this port in order to avoid a conflict.For adoption (brownfield), dashboard port should be updated to 8443 in ceph-mgr config and firewall rules.

. Update dashboard port in ceph mgr config to 8443 using the following steps
+
----
$ sudo cephadm shell
