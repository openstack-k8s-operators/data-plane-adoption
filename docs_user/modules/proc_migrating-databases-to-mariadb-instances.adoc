...
* Ensure that there is network routability between the original MariaDB and the MariaDB for the control plane. i.e rechable via ping from the adoption running node.
...
ifeval::["{build_variant}" == "ospdo"]
----
$ oc -n $OSPDO_NAMESPACE wait --for condition=Ready pod/mariadb-copy-data --timeout=30s
----
endif::[]
ifeval::["{build_variant}" != "ospdo"]
...
ifeval::["{build_variant}" == "ospdo"]
>     oc -n "${OSPDO_NAMESPACE} rsh mariadb-copy-data mysql \
endif::[]
...
----
$ for CELL in $(echo "super $RENAMED_CELLS"); do
>   oc -n ${RHOSO18_NAMESPACE} run mariadb-client -n $NAMESPACE --image $MARIADB_IMAGE -i --rm --restart=Never -- \
...
ifeval::["{build_variant}" == "ospdo"]
>   oc -n "${OSPDO_NAMESPACE}" rsh mariadb-copy-data << EOF
endif::[]
...
ifeval::["{build_variant}" == "ospdo"]
$ dbs=$(oc exec openstack-galera-0 -n $RHOSO18_NAMESPACE -c galera -- mysql -rs -uroot -p"${PODIFIED_DB_ROOT_PASSWORD['super']}" -e 'SHOW databases;')
endif::[]
...
ifeval::["{build_variant}" == "ospdo"]
$ novadb_mapped_cells=$(oc exec openstack-galera-0 -n $RHOSO18_NAMESPACE -c galera -- mysql -rs -uroot -p"${PODIFIED_DB_ROOT_PASSWORD['super']}" \
endif::[]
...
ifeval::["{build_variant}" == "ospdo"]
>   c1dbs=$(oc exec openstack-$CELL-galera-0 -n $RHOSO18_NAMESPACE -c galera -- mysql -rs -uroot -p${PODIFIED_DB_ROOT_PASSWORD[$CELL]} -e 'SHOW databases;') <4>
endif::[]
