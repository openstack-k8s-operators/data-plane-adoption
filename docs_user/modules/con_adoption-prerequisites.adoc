...
* If the `systemd-container` package is not installed on your Compute hosts, install it by using the following command and then reboot all hypervisors one by one to activate the `systemd-container` package:
+
[source, shell]
----
