...
* Make sure to set the correct {rhos_acro} project namespace to execute commands in.
ifeval::["{build_variant}" == "ospdo"]
In Director Operator adoption the adopted namespace would be "openstack", the adopting namespace would be different i.e. "rhoso".
endif::[]
[source, shell]
----
ifeval::["{build_variant}" == "ospdo"]
...
* If the `systemd-container` package is not installed on your Compute hosts, install it by using the following command:
+
[source, shell]
----
