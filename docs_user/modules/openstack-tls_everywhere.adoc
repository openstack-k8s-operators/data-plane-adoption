...
The final step on this activity is to stop and disable the certmonger service, and remove all certificates managed by it. The code below accomplishes this task:

[source, bash]
----
#!/bin/bash

if sudo systemctl is-active certmonger.service; then
    echo "Stopping certmonger..."
    sudo systemctl stop certmonger.service
    echo "Disabling certmonger..."
    sudo systemctl disable --now certmonger.service
fi

# If necessary, adjust the directory below accordingly.
certs_directory=/var/lib/certmonger/requests/
certs_to_stop=$(ls -1 $certs_directory)
number_of_certs=$(ls -1 $certs_directory | wc -l)

if [ $? != 0 ] || [ $number_of_certs = 0 ]; then
    echo "No certificates to be removed."
else
    echo "There is/are $number_of_certs certificates to be removed. Removing certificates..."
fi
