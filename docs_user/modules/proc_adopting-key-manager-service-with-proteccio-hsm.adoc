...
* The `BarbicanAPI`, `BarbicanWorker`, and `BarbicanKeystoneListener` services are up and running with Proteccio custom images.
...
** Your HSM Certificate Authority certificate file (filename varies by environment)
...
[IMPORTANT]
====
The `ansible-role-rhoso-proteccio-hsm` role is **mandatory** for Proteccio HSM adoption. This role creates essential Kubernetes secrets and configures HSM integration. The adoption process will fail if this role is not available or cannot execute successfully.
