...
* Ensure that there are no ongoing instance live migrations, volume migrations (online or offline), volume creation, backup and restore, attaching, detaching, and so on:
+
----
...
. Remove any constraints between infrastructure and {OpenStackShort} control plane
services.

. The `cinder-backup` service on {OpenStackShort} {rhos_prev_ver} could be running as Active-Passive under Pacemaker or as Active-Active. You must check how it is running and stop it.

. If your deployment enables CephFS through NFS as a backend for {rhos_component_storage_file_first_ref}, there are Pacemaker ordering and co-location constraints that govern the Virtual IP address that is assigned to the `ceph-nfs` service and the `manila-share` service. Remove these constraints:
+
----
...
* Verify that the control plane services are stopped.
