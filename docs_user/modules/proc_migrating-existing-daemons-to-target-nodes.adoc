...
+
[NOTE]
The Ceph Dashboard, as the service  provided by the Ceph `mgr`, is not impacted by the relocation. You might experience an impact when the active `mgr` daemon is migrated or is force-failed. However, you can define 3 replicas to redirect requests to a different instance (it’s still an A/P model), so that the impact is limited.
