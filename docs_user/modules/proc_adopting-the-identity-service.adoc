...
. Replace the loadBalancerIPs with the appropriate VIP,
  Wait for openstackcontrolplane to be Ready:
+
----
  oc wait openstackcontrolplane openstack -n ${RHOSO18_NAMESPACE} --for condition=Ready --timeout=600s || {
