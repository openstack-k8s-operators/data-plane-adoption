...
. Verify that the setup of `OpenStackControlPlane` CR is now complete:
+
----
...
. Using the `OpenStackClient` pod remote shell, verify that the {identity_service}
endpoints are defined and are pointing to the control plane FQDNs:
+
----
