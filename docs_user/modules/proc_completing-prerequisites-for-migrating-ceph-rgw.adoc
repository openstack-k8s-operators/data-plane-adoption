...
You must complete the following prerequisites before you begin the Ceph Object Gateway (RGW) migration.
...
. Identify the ranges of the storage networks:
+
----
...
. Identify the network that you previously had in HAProxy and propagate it through {OpenStackPreviousInstaller} to the {CephCluster} nodes. This network is used to reserve a new VIP that is owned by {Ceph} and used as the entry point for the RGW service.
...
.. Confirm that the network is used as an HAProxy front end. The following example shows that `controller-0` exposes the services by using the external network, which is not present in the {Ceph} nodes. You must propagate the external network through {OpenStackPreviousInstaller}:
+
----
...
.. Change the NIC template that is used to define the `ceph-storage` network interfaces and add the new config section:
+
[source,yaml]
----
---
