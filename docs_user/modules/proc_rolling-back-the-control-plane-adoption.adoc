...
If you encountered a problem that prevents you from completing the adoption of the {rhos_prev_long} ({OpenStackShort}) control plane services, you can roll back the control plane adoption.

[IMPORTANT]
You can only roll back the control plane adoption during the parts of the adoption procedure that alter the control plane. If you altered the data plane
nodes in any way, you cannot perform the rollback procedure.
...
can run `openstack` CLI commands or check that you can access the {dashboard_first_ref}.
...
state might have changed. Before you retry the adoption procedure, verify that all the control plane resources are removed and that there are no leftovers which could affect the following adoption procedure attempt. You must not use previously created copies of the database contents in another adoption attempt. You must make new copies. For more information about making new copies, see xref:migrating-databases-to-the-control-plane_storage-requirements[Migrating databases to the control plane].
