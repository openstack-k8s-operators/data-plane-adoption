...
----
+
The resources provided in the `ConfigMap` contain cell-specific configurations.
...
+
[NOTE]
Ensure that you retain the list of services from the original `OpenStackDataPlaneNodeSet` CR that you deployed, except for the inserted `ceph-client` service.
...
.. Create the validation service. For example:
+
[source,yaml]
----
$ oc apply -f - <<EOF
...
.. Create a `OpenStackDataPlaneDeployment` CR that runs only the validation. For example:
+
[source,yaml]
----
$ oc apply -f - <<EOF
...
add them in the `nodeSets` list in this step, or create separate `OpenStackDataPlaneDeployment` CRs later. Note that once a `OpenStackDataPlaneDeployment` CR is deployed, you cannot add new node sets to it later.
