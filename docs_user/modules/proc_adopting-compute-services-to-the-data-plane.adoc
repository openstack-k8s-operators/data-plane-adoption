...
+
----
$ oc -n $RHOSO18_NAMESPACE create secret generic redhat-registry \
...
[source,yaml]
----
$ oc -n $RHOSO18_NAMESPACE patch osdpns/openstack-cell1 --type=merge --patch "
spec:
  services:
ifeval::["{build}" == "downstream"]
    - redhat
...
[source,yaml]
----
$ oc -n $RHOSO18_NAMESPACE patch openstackdataplanenodeset openstack-cell1 --type='json' --patch='[
  {
    "op": "add",
    "path": "/spec/services/-",
    "value": "neutron-sriov"
...
+
----
$ watch oc -n $RHOSO18_NAMESPACE get pod -l app=openstackansibleee
----
+
----
$ oc -n $RHOSO18_NAMESPACE logs -l app=openstackansibleee -f --max-log-requests 20
----
...
+
----
$ watch -n $RHOSO18_NAMESPACE oc get pod -l app=openstackansibleee
----
+
----
$ oc -n $RHOSO18_NAMESPACE logs -l app=openstackansibleee -f --max-log-requests 20
...
+
----
$ oc -n $RHOSO18_NAMESPACE wait --for condition=Ready osdpns/openstack-cell1 --timeout=30m
----

. Verify that the {networking_first_ref} agents are running:
+
----
$ oc -n $RHOSO18_NAMESPACE exec openstackclient -- openstack network agent list
+--------------------------------------+------------------------------+------------------------+-------------------+-------+-------+----------------------------+
...
. Verify that the hypervisors are UP:
+
----
$ oc -n $RHOSO18_NAMESPACE exec openstackclient -- openstack compute service list
$ oc -n $RHOSO18_NAMESPACE exec openstackclient -- openstack hypervisor list
----
+
. Verify that the VMs are running / started:
+
----
$ oc -n $RHOSO18_NAMESPACE exec openstackclient -- openstack server lis
----
