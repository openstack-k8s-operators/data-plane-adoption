...
. For each Compute node grab the UUID of the Compute service and write it to the stable `compute_id` file in `/var/lib/nova/` directory.
+
[subs=+quotes]
----
...
+
Make sure that ovn-controller settings configured in the `OpenStackDataPlaneNodeSet` CR are the same as were set in the Compute nodes before adoption.
...
+
`Neutron-ovn-metadata-agent` running on the data plane nodes do not require any additional actions nor configuration adjustments to do during the adoption process.
When `OpenStackDataPlaneDeployment` and `OpenStackDataPlaneNodeSet` will be ready, `neutron-ovn-metadata-agent` should be up and running on the data plane nodes.
