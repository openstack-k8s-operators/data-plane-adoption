...
* You have stopped the remaining control plane nodes, repositories, and packages on the {compute_service_first_ref} hosts. For more information, see xref:proc_stopping-infrastructure-management-and-compute-services.adoc#stopping-infrastructure-management-and-compute-services_{context}[Stopping infrastructure management and Compute services].
...
`spec.nova.template.cellTemplates.cell<X>.metadataServiceTemplate.enable` in the `OpenStackControlPlane/openstack` CR, as described in xref:proc_adopting-the-compute-service.adoc#adopting-the-compute-service_{context}[Adopting the Compute service]. You can configure a single top-level metadata, or define the metadata per cell.
...
* You must perform a fast-forward upgrade on your Compute services. For more information, see xref:proc_performing-a-fast-forward-upgrade-on-compute-services.adoc#performing-a-fast-forward-upgrade-on-compute-services_{context}[Performing a fast-forward upgrade on Compute services].
