...
. Add the RGW label to the {Ceph} nodes to which to migrate your RGW back ends:
+
----
for i in 0 1 2; {
...
. During the overcloud deployment, RGW is applied at step 2
(external_deployment_steps), and a `cephadm` compatible spec is generated in
`/home/ceph-admin/specs/rgw` from {OpenStackPreviousInstaller}. Locate the RGW spec:
endif::[]
+
----
...
. Ensure that the new RGW back ends are reachable on the new ports. You are eventually going to enable an IngressDaemon on port 8080. Log in to each {CephCluster} node that includes RGW and add the `iptables` rule to allow connections to both 8080 and 8090 ports in the {CephCluster} nodes:
+
----
...
. If RGW back ends are migrated to the {Ceph} nodes, there is no "`internalAPI`" network. You must reconfigure the RGW keystone endpoint to point to the external network that you propagated: 
+
----
