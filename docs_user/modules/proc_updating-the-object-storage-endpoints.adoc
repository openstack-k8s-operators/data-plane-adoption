...
= Updating the object storage endpoints

The object-storage endpoints still point to the original virtual IP address (VIP) that is owned by Pacemaker. 
You must update the object-storage endpoints to point to the new VIP because other services stll use the original VIP, and you reserved a new virtual IP address (VIP) that you reserved on the same network.
