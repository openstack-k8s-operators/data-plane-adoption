...
assigned to the target nodes, where the daemons are scheduled. Review the https://access.redhat.com/articles/1548993[cardinality matrix] before assigning labels and choose carefully the nodes where the monitoring
stack components should be scheduled on.
//kgilliga: Should this "cardinality matrix" article be available only downstream?
+
----
