...
[NOTE]
In addition to updating the container images related to the monitoring stack, on a Red Hat Ceph Storage  director-deployed environment, you must update the configuration entry related to the `container_image_base`. This has an impact on all the {Ceph} daemons that rely on the undercloud images.
