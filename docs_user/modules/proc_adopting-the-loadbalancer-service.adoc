...
+
----
$ alias openstack="oc -n $RHOSO18_NAMESPACE exec -t openstackclient -- openstack"
----
...
+
----
$ oc get -n -n $RHOSO18_NAMESPACE  --no-headers nncp | cut -f 1 -d ' ' | while read; do

interfaces=$(oc -n $RHOSO18_NAMESPACE  get nncp $REPLY -o jsonpath="{.spec.desiredState.interfaces[*].name}")

(echo $interfaces | grep -w -q "octbr|enp6s0.24") || \
        oc patch -n $RHOSO18_NAMESPACE  nncp $REPLY --type json --patch '
[{
    "op": "add",
...
EOF_CAT
$ oc apply -n $RHOSO18_NAMESPACE  -f octavia-nad.yaml
----

. Enable the {loadbalancer_service} in {OpenShiftShort}:
+
----
$ oc patch openstackcontrolplane openstack -n $RHOSO18_NAMESPACE  --type=merge --patch '
spec:
  ovn:
    template:
      ovnController:
...
+
----
$ oc -n $RHOSO18_NAMESPACE  wait --for condition=Ready --timeout=600s octavia.octavia.openstack.org/octavia
----
