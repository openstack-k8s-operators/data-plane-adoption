...
+
These commands copy the existing public SSH key that that you can use for connecting to the amphorae and installs it into Openshift.
+
[source,bash]
----
...
Change `ONBOOT=yes` to `ONBOOT=no` to ensure that the interface is disabled on reboot:
...
+
Store the network ID of the old management network in the
variable `WALLABY_LB_MGMT_NET_ID` for later use:
+
----
