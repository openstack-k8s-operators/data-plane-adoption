...
* You can keep the mon IP addresses by moving them to the Red Hat Ceph Storage nodes.
* You can drain the existing controller nodes and shut them down.
* You can deploy additional monitors to the existing nodes, and promote them as
...
//Note to self: Keeping this here in case I rewrite it and use it in the intro: The goal of the next section is to migrate the oc0-controller-{1,2} daemons into oc0-ceph-{0,1} as the very basic scenario that demonstrates that you can actually make this kind of migration using cephadm.
...
. On the oc0-ceph-0: 
//kgilliga: What is happening in this step?
+
----
...
. Power off oc0-controller-1.
//kgilliga: Is this something that customers just know how to do?
