...
[NOTE]
====
If you backed up your {rhos_prev_long} ({OpenStackShort}) services configuration file from the original environment, compare it with the `customServiceConfig` that you apply in `glance_patch.yaml` and ensure that the configuration is correct.
====
