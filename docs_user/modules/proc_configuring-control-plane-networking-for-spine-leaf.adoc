...
[role="_abstract"]
If you are adopting a spine-leaf or Distributed Compute Node (DCN) deployment, update the control plane networking for communication across sites. Add subnets for remote sites to your existing `NetConfig` CR and update `NetworkAttachmentDefinition` CRs with routes to enable connectivity between the central control plane and remote sites.
...
spec:
  ...
  - name: tenant
    ...
    - name: tenantsite2
      ...
      vlan: 42
----
* Replace `<subnet1>` with a user-defined subnet name for the central site subnet.
* Replace `<ctlplanesite1>` with a user-defined subnet for the first DCN edge site.
* Replace `<ctlplanesite2>` with a user-defined subnet for the second DCN edge site.
+
[NOTE]
====
You must have the `storagemgmt` network on OpenShift nodes when using DCN with Swift storage. It is not necessary when using Ceph.
====
