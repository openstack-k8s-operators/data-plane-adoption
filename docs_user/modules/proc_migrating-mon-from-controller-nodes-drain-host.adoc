...
. Fail the `ceph-mgr` if it is active on the source or draining node :
+
----
...
. (Optional) Ensure to remove Ceph Monitor daemon from the source node if it is still running ):
+
----
