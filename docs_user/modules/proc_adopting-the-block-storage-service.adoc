...
* Replace `<patch_name>` with the name of your patch file.
+
* The following example shows a `cinder_services.patch` file for an Ceph RBD deployment:
+
[source,yaml]
----
spec:
...
----
+
* Configure Netapp Nfs cinder volume service:
+
* Create secrets for Netapp Nfs backend that includes sensitive information like hostnames, passwords and usernames to access the thrid party Netapp Nfs storage.
  Find the credentials from the cinder.conf generated from tripleo deployment.
[source,yaml]
----
$ oc apply -f - <<EOF
apiVersion: v1
...
----
+
* The following example shows  `cinder_netappNFS.patch` file for configuring a Netapp Nfs cinder volume service:
...
----
+
* Patch the `OpenStackControlPlane` CR to deploy Netapp Nfs cinder volume backend.
----
