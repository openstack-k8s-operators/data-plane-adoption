...
[NOTE]
====
Always ensure to use the same configuration group name for the driver as in the source cluster. In the example above, the driver configuration group in `customServiceConfig` is called `tripleo_ceph` because it reflects the value of the configuration group name found in the cinder.conf file of the source OpenStack cluster.
====
+
