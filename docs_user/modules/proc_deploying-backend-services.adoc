...
* If you enabled TLS Everywhere on the {OpenStackShort} environment, you must copy the `tls` root CA from the {OpenStackShort} environment to the `rootca-internal` issuer.
...
synchronizing the service account passwords is required.
+
...
. Deploy the `OpenStackControlPlane` CR. Ensure that you only enable the DNS,
MariaDB, Memcached, and RabbitMQ services. All other services must
be disabled.

. If you enabled TLS Everywhere in your {OpenStackShort} environment, modify the `spec:tls` section with the following override before applying it:
