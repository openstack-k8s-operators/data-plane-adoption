...
Create the `OpenStackControlPlane` custom resource (CR) with basic backend services deployed, and all the {osp_prev_long} ({OpenStackShort}) services disabled. This will be the foundation of the control plane.
...
{OpenStackShort} {osp_prev_ver} release.
