...
* If the deployment uses CephFS through NFS as a storage back end, the
...
* The {OpenShift} nodes where the `manila-share` service is to be deployed
...
* If you enabled tenant-driven networking (`driver_handles_share_servers=True`),
the {networking_first_ref} is deployed.
...
. Review the configuration file alongside any configuration changes that were made since {OpenStackShort} {rhos_prev_ver}. Not all of it makes sense to bring into the new cloud environment:
...
`customServiceConfigSecrets` key. For example:
+
[source,yaml]
----
