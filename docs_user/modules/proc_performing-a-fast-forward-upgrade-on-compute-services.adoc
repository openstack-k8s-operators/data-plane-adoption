...
{compute_service_first_ref} operator and OpenStack Operator ensure upgrading
...
Note that in this guide, the cells databases are using the same password defined in `osp-secret`.

.Procedure

. Wait for Compute data plane services version updated for all cells (it may take some time):
+
----
for CELL in $(echo $RENAMED_CELLS); do
...
. Wait for Compute data plane services to be ready for all cells:
+
----
