...
----
+
* If you are adopting the {compute_service} with the {bare_metal_first_ref}, append the following `novaComputeTemplates` in the `cell<N>` section of the {compute_service} CR patch:
+
[source,yaml]
----
        cell<N>:
...
----
+
<1> Replace `<hostname>` with the hostname of the node that is running the `ironic` Compute driver in the source cloud of `cell<N>`.
