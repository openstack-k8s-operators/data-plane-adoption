...
You must upgrade the Compute services from {rhos_prev_long} {rhos_prev_ver} to {rhos_long} {rhos_curr_ver} on the control plane and data plane. The upgrade procedure involves the following tasks:
...
. Wait for cell1 Compute data plane services version to be updated:
+
----
...
If you encounter errors, review the nova Compute agent log files on the data plane, and the `nova-conductor` journal records on the control plane.
====
