...
$ for CELL in $(echo $RENAMED_CELLS); do
  oc -n $RHOSO18_NAMESPACE exec openstack-$CELL-galera-0 -n $NAMESPACE -c galera -- mysql -rs -uroot -p"${PODIFIED_DB_ROOT_PASSWORD[$CELL]}" \
...
[source,yaml]
----
$ oc -n $RHOSO18_NAMESPACE patch openstackcontrolplane openstack -n openstack --type=merge --patch '
spec:
  nova:
    template:
      cellTemplates:
...
+
----
$ oc -n $RHOSO18_NAMESPACE wait --for condition=Ready --timeout=300s Nova/nova
----

. Make sure to complete the steps in xref:adopting-compute-services-to-the-data-plane_data-plane[Adopting Compute services to the {rhos_acro} data plane].
...
+
----
$ oc -n $RHOSO18_NAMESPACE wait --for condition=Ready openstackdataplanedeployment/openstack-nova-compute-ffu --timeout=5m
----

. Run Compute database online migrations to complete the fast-forward upgrade:
+
----
$ oc -n $RHOSO18_NAMESPACE exec -it nova-cell0-conductor-0 -- nova-manage db online_data_migrations
$ oc -n $RHOSO18_NAMESPACE exec -it nova-cell1-conductor-0 -- nova-manage db online_data_migrations
----
