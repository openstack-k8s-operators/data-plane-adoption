...
+
----
$ oc -n $RHOSO18_NAMESPACE patch openstackcontrolplane openstack --type=merge -p='{"spec":{"swift":{"template":{"swiftStorage":{"replicas": 3}}}}}'
----
+
...
+
----
$ oc -n $RHOSO18_NAMESPACE wait pods --for condition=Ready -l component=swift-storage
$ oc -n $RHOSO18_NAMESPACE debug --keep-labels=true job/swift-ring-rebalance -- /bin/sh -c 'swift-ring-tool get && swift-ring-builder object.builder search --device pv'
----
