...
. Log in to a Controller node and verify that the `auth_encryption_key` value is the same as the `HeatAuthEncryptionKey` value:
+
----
...
. Verify that you can see your {orchestration} stacks. Test whether you can create networks, subnets, ports, or routers:
+
----
