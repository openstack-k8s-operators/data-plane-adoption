...
To enable local cells metadata services, set `spec.nova.template.cellTemplates.cell*.metadataServiceTemplate.enable`
in `OpenStackControlPlane` CR.
        ...
        cell0:
          conductorServiceTemplate:
            customServiceConfig: |
              [workarounds]
              disable_compute_service_check_for_ffu=true
        cell1: # Copy this data into new sections defining the needed cells!
          metadataServiceTemplate:
...
----
+
Copy `cellTemplates.cell1` data into new sections defining the needed cells.
...
+
*NOTE*: Set the `[DEFAULT]host` configuration option to match the hostname of the node(s) running the `ironic` compute driver in the source cloud, depending on a deployed topology.
For an example single-cell standalone deployment:
+
[source,yaml]
----
        cell1:
...
* Query the superconductor for renamed cells existance, and compare it to pre-adoption values:
+
----
...
for CELL in $(echo $CELLS); do
  ...
  echo $PULL_OPENSTACK_CONFIGURATION_NOVAMANAGE_CELL_MAPPINGS[$CELL]
  oc rsh nova-${RCELL}-conductor-0 nova-manage cell_v2 list_cells | grep -F '| $RCELL |'
done
----
+
The expected changes to happen:

 ** The `cellX` cell's `nova` DB and user name should change to `nova_cellX`.
 ** The `default` cell should become renamed to `DEFAULT_CELL_NAME` (it may contain the original name, if there are multiple cells).
