...
To enable the metadata services of a local cell, set the `enabled` property in the`metadataServiceTemplate` field to `true` in the `OpenStackControlPlane` CR.

//Bohdan: Please confirm that my rewrite above is accurate.
+
----
rm -f celltemplates
...
----
//Bohdan: Do we need this note in the code? # deploy single nova metadata on the top level
//Also, I added command prompts ($) before each command. Please confirm that this looks accurate.
+
...
//Bohdan: I removed <index> from the following code because I clarified above that this is an example. Let me know if you have concerns.
+
[source,yaml]
----
        cell1:
