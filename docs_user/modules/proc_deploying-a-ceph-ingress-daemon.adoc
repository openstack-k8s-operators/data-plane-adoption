...
. Restart the service to ensure that other services that consume the {object_storage_first_ref} are not affected by this change.
...
----
+
The {object_storage_first_ref} CLI fails to establish the connection. This is expected:
+
----
