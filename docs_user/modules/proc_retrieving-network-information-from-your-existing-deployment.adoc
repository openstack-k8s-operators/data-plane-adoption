...
. In the `network_data.yaml` file, make a note of the VLAN tag that is used in the `vlan` key and the IP range in the `ip_subnet` key for each isolated network. The IP range is later split into separate pools for control plane services and load balancer IP addresses.
