...
If you are using OpenStack Swift as the {object_storage_first_ref}, adopt the {object_storage} to the {rhos_long} environment. If you are using the Object Storage API of the Ceph Object Gateway (RGW), skip this procedure.

.Prerequisites

* Ensure that the {object_storage} storage back-end services are running.
* Ensure that the storage network is properly configured on the {OpenShift} cluster.
...
* Ensure that the Object Storage proxy service is registered in the {identity_service}:
+
----
...
* Confirm that you are able to upload and download objects:
+
----
