...
** If the {image_service} is configured to use {block_storage} as a backend with FC, the {image_service} must also run on an {OpenShiftShort} host that has HBAs and follow the same node selection requirements as the {block_storage}.
