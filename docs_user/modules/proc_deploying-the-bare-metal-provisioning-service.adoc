...
+
----
$ oc -n $RHOSO18_NAMESPACE wait --for condition=Ready --timeout=300s ironics.ironic.openstack.org ironic
----

. Verify that the individual services are ready:
+
----
$ oc -n $RHOSO18_NAMESPACE wait --for condition=Ready --timeout=300s ironicapis.ironic.openstack.org ironic-api
$ oc -n $RHOSO18_NAMESPACE wait --for condition=Ready --timeout=300s ironicconductors.ironic.openstack.org ironic-conductor
$ oc -n $RHOSO18_NAMESPACE wait --for condition=Ready --timeout=300s ironicinspectors.ironic.openstack.org ironic-inspector
$ oc -n $RHOSO18_NAMESPACE wait --for condition=Ready --timeout=300s ironicneutronagents.ironic.openstack.org ironic-ironic-neutron-agent
----
