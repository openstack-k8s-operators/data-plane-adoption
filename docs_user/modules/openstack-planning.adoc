...
The xref:tls-everywhere__{context}[TLS everywhere document] describes how to extract
the root CA from IdM in the source environment and import it for use by certmanager
in the new environment. Because the same root CA is used to issue new certificates,
we can minimize disruption on the compute nodes because the currently used chain of
trust does not need to be modified.
