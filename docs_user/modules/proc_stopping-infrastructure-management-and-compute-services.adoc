...
You must stop cloud Controller nodes, database nodes, and messaging nodes on the {rhos_prev_long} {rhos_prev_ver} control plane. However, nodes that are running the Compute, Storage, or Networker roles must remain functional on the control plane.
