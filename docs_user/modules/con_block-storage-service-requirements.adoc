...
= {block_storage} requirements

To prepare to adopt your {block_storage_first_ref}, review the following requirements:

* Evaluate your {block_storage} back ends.
* Check the transport protocol that the {block_storage} back ends use, such as
...
* To deploy {block_storage} volume back ends, use a {block_storage} volume service for each back end.
+
